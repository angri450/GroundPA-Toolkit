...
```powershell
# 构建检索式
nong lit parse "SU=玉米 AND SU=抗旱性 AND YE BETWEEN(2023,2026) - SU=综述" --json

# 验证
nong lit validate "SU=玉米 AND SU=抗旱性 AND YE BETWEEN(2023,2026) - SU=综述" --json

# 查看检索计划（看会用哪些 provider、生成多少查询）
nong lit plan "SU=玉米 AND SU=抗旱性 AND YE BETWEEN(2023,2026) - SU=综述" --json

# 执行检索
nong lit search "SU=玉米 AND SU=抗旱性 AND YE BETWEEN(2023,2026) - SU=综述" --json
```
